...
func formatContacts(input map[types.JID]types.ContactInfo) (result []string) {
	for jid, contact := range input {
		if len(contact.FullName) == 0 {
			continue
		}
		result = append(result, fmt.Sprintf("* %s / %s - `+%s`", contact.FullName, contact.PushName, jid.User))
	}
...
func (handler *CommandHandler) CommandList(ce *CommandEvent) {
	...
	if contacts {
		...
		result = formatContacts(contactList)
